...
const MenuItemsByRestaurant = () => {
    ...
    useEffect(() => {
        const fetchMenuItems = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/api/menu-items/restaurant/${restaurantId}`);
                setMenuItems(response.data || []);
            } catch (error) {
                setError('Error fetching menu items by restaurant.');
                            ...
                            {menuItems.slice(0, 6).map(item => (
                                <div key={item.id} className="p-5">
                                    <div className="bg-white rounded-lg shadow-md p-4 flex flex-col items-center hover:shadow-lg transition transform hover:-translate-y-1">
                                        ...
                                        <button
                                            className="bg-blue-500 text-white px-4 py-2 mt-3 rounded-md w-full font-medium shadow-md hover:bg-blue-600 transition-transform transform hover:scale-105"
...
export default MenuItemsByRestaurant;
